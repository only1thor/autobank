...
{#if open}
	<div class="fixed inset-0 z-50 overflow-y-auto">
		<div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
			<!-- Backdrop -->
			<div
				class="fixed inset-0 bg-gray-8000/75 transition-opacity"
				onclick={oncancel}
				role="button"
				tabindex="-1"
			...
			>
				<div class="bg-gray-800 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
					<div class="sm:flex sm:items-start">
						...
						<div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
							<h3 class="text-base font-semibold leading-6 text-gray-100">{title}</h3>
							<div class="mt-2">
				...
				<div class="bg-gray-800 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
					<button
						type="button"
						class="btn {variant === 'danger' ? 'btn-danger' : 'btn-primary'} sm:ml-3"
						onclick={onconfirm}
					>
						{confirmText}
					</button>
					<button type="button" class="btn btn-secondary mt-3 sm:mt-0" onclick={oncancel}>
						{cancelText}
					</button>
				</div>
			</div>
