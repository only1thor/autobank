	...
	<div class="flex items-center gap-4">
		...
		{#if account}
			<div>
				...
				<p class="text-gray-500">{account.accountNumber}</p>
			</div>
		{:else}
			<div>
	...
	{:else if account}
		...
		<div class="card p-6">
				...
				<div class="flex-1">
					<div class="flex items-center justify-between">
						<div>
							...
							<p class="text-sm text-gray-500">IBAN: {account.iban}</p>
						</div>
			...
			<div class="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t">
				<div>
					<div class="text-xs text-gray-500 uppercase">Product</div>
					<div class="text-sm font-medium">{account.productType}</div>
				</div>
				<div>
					<div class="text-xs text-gray-500 uppercase">Currency</div>
					<div class="text-sm font-medium">{account.currencyCode}</div>
				</div>
				<div>
					<div class="text-xs text-gray-500 uppercase">Transfer From</div>
					<div class="text-sm font-medium">
						{account.accountProperties.isTransferFromEnabled ? 'Yes' : 'No'}
					</div>
				</div>
				<div>
					<div class="text-xs text-gray-500 uppercase">Transfer To</div>
					<div class="text-sm font-medium">
